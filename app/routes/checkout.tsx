...
export async function action({ request, context }: Route.ActionArgs): Promise<ActionResult> {
  ...
  if (!await getCustomerSession(request, itemsStore)) return { ok: false, message: "Please verify your phone number to proceed with your order." };
  ...
  try {
    ...
    return { ok: true, orderId: result.order.id, total: result.order.total, paymentMethod, giftIncluded: result.order.giftIncluded === 1, lowStock: result.lowStock };
  } catch (error) {
    try {
