...
function CustomerLoginModal({ variant, notice, onClose, onSuccess }: { variant: "timed" | "checkout"; notice: string; onClose: () => void; onSuccess: (user: CustomerSession) => void }) {
  ...
  const [stagingDebugOtp, setStagingDebugOtp] = useState<string | null>(null);
  ...
  async function sendOtp() {
    ...
    try {
      ...
      const result = await response.json() as { ok: boolean; message?: string; delivery?: string; debugOtp?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "OTP could not be sent.");
      setStagingDebugOtp(result.debugOtp ?? null);
      setPhone(localPhone); setStep(2); setCooldown(30); setMessage(result.debugOtp ? "Staging test OTP is shown in the banner above." : result.delivery === "console" ? "Staging OTP fallback is active; check Worker logs for the code." : "OTP sent. It expires in 5 minutes.");
